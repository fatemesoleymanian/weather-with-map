<template>
  <main class="container text-white">
    ...
    <div class="search-box pt-10  mb-8 relative px-12 md:px-14 lg:px-32">
      ...
      placeholder="Search for a city or state"  v-on:keyup.enter="previewmyLocation"
      ...
      <i class="fas fa-dot-circle absolute lg:right-36 top-16 md:right-20 right-16 cursor-pointer "
        @click="currentCity"></i>
      <ul class="absolute bg-weather-secondary w-3/4 md:w-100 text-white shadow-md py-2 px-1 top-[102px]"
        style="z-index: 90;" v-if="mapboxSearchResults">
...
export default {
  ...
  methods: {
    ...
    previewmyLocation() {
        this.router.push({
          name: "cityView",
          params: { state: this.searchQuery, city: this.searchQuery },
          query: {
            lat: this.lat,
            lng: this.lon,
            preview: true,
    ...
    getSearchResults() {
      ...
      this.queryTimeout = setTimeout(async () => {
        ...
        if (this.searchQuery !== "") {
          try {
            const result = await axios.get(`${this.mapBoxBaseUrl}${this.searchQuery}.json?access_token=${this.mapboxAPIKey}&types=place`);
  ...
  components: { Map ,CityCardSkeleton , CityList , Suspense }
}
