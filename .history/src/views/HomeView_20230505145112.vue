<template>
  <main class="container text-white">
    ...
    <div class="search-box pt-10  mb-8 relative px-12 md:px-14 lg:px-32">
      ...
      <!-- Map -->
      <div>
        <mgl-map mapStyle="https://api.maptiler.com/maps/streets/style.json?key=tZVs0ZFomP3biIShjxuD" :zoom=2
          :center="[lon, lat]">
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (position) => {
            ...
            try {
              const city = await axios.get(`http://api.openweathermap.org/geo/1.0/reverse?lat=${lat}&lon=${lon}&limit=1&appid=eb0ed08b5e5fa33a9c9cc66ad7e2f028`)
              searchQuery.value = city.data[0].name;
        ...
        if (searchQuery.value !== "") {
          try {
            const result = await axios.get(
              `${mapBoxBaseUrl}${searchQuery.value}.json?access_token=${this.mapboxAPIKey}&types=place`
            );
